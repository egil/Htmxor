...
- Keep lower-level Blazor render-tree generation framework-owned. Private framework access is limited to the isolated, monitored [approved framework adapter](docs/roadmap/v1/goal.md#form-service-adapter) for form, initializer, and resource seams; each use requires exact upstream provenance, source inventory, drift monitoring, runtime shape validation, and paired parity evidence. Do not copy private renderer code.
