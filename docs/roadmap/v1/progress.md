...
- Framework boundary under test: ASP.NET Core 10.0.11 and Blazor static SSR. Issues #95, #97, #100, #103, #106, #125, and #141 use a separate external .NET 10 Razor consumer on TestServer that restores a locally packed `net8.0` Htmxor package instead of referencing an Htmxor project. Issues #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, and #139 use a separate package-only .NET 10 application on real Kestrel; its browser cases use Chromium, while issues #18, #50, and #127 also use `HttpClient` for server-response assertions. Issues #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, and #139 publish that external application before running its tests from the publish output in Production.
...
- Issues #95, #97, #100, #103, #106, #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, #139, and #141 prove one locally packed package with the current SDK and dependency set. Issues #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, and #139 additionally prove a framework-dependent application publish, but none proves a NuGet-published or signed package, a release candidate, package compatibility across SDK or compiler versions, or a broader target-framework matrix.
...
Issue #141 establishes the current route-cardinality contract:

> When a package-consuming .NET 10 Blazor static-SSR application declares three
> otherwise-supported project-root `HtmxRoute` components and calls
> application-level Htmxor registration once, Htmxor maps all three without an
> arbitrary component-count diagnostic, while each route retains its own method,
> authorization, host, route-group, and component metadata.

The external .NET 10 TestServer consumer restores a locally packed Htmxor package
and retains the existing Razor-backed report and all-C# summary routes as
controls beside a third project-root Razor audit route. One application
registration maps exactly those three direct-only routes in deterministic
component identity order. The package proof exercises route-specific methods,
policies, hosts, component metadata, shared group metadata, direct isolation,
normal-route rejection, and anonymous, wrong-policy, wrong-host, and route-value
constraint controls while retaining the existing generated stock actions. The
analyzer and runtime catalog no longer reject a supported declaration solely
because another supported component exists; all per-component validation remains
fail closed.

The recommended next slice remains a separately scoped concurrent-request
isolation proof at the package/Kestrel/Chromium boundary. It should use two
request-unique selected gates to establish that releasing one request cannot
complete, swap, or leak lifecycle observations into the other, while each
request's excluded sibling still records zero work. Nested or multiple selection
within one response, benchmarks, caching, streaming, errors, forms, and unsafe
methods remain separate slices.
