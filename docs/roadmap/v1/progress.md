# Htmxor v1 progress — historical archive
...
- Verified green characterization commit for issue #135: `f4b0d2abe3b51d22fb536f6058ff58953f2f8c1c`, based on exact freshly fetched `origin/main` commit `ee52f903a3340c1acc185c2a96fb1a23b4112ca6`.
- Verified post-Copilot observer proof commit for issue #135: `fdf3155caef3a8d6fe9aa070030c41d29b46e124`.
- This issue #135 progress change is documentation-only. Executable claims are tied to the tested commits above, not to the later documentation head.
- Verified green characterization commit for issue #137: `a33d9e4635f15812329634a2f48f131e0e5ee3e4`, based on exact freshly fetched `origin/main` commit `06c2558b3bae6e19691d804382005dcd1f26738b`. No production change was required.
- Verified post-review swap-observation proof for issue #137: `3b046f57dacc85e87b2a5f752eea0f6f6510b75c`.
- This issue #137 progress change is documentation-only. Executable claims are tied to the tested characterization commits above, most recently the post-review proof, not to the later documentation head.
- Verified green characterization commit for issue #139: `030e3d9830dfab70022920afeac99498d9ceb85a`, based on exact freshly fetched `origin/main` commit `28418a4995827a550aed1253576a1108b7c98d5a`. No production change was required.
- Verified Standards-review quiescence-observation fix for issue #139: `397756dd3a49b30fb56fac4c218269a26434ede8`.
- Preserved meaningful package/compiler red for issue #141: `a3a79143ab294e4b808c3606fa54b13ca29b11bf`, based on exact freshly fetched `origin/main` commit `a082e77c52903cbbef06d3a3e511d1a15fc41f50`.
- Verified executable proof commit for issue #141: `a90d72e63a1a88d35da8e344584df1f0a4a112f5`.
- This issue #141 progress change is documentation-only. Executable claims are tied to the tested proof commit above, not to the later documentation head.
- Verified post-review green characterization commit for issue #144: `36183cc64c97a55ee816e454b3d7954cfbe0c7e2`, based on exact freshly fetched `origin/main` commit `ca5a00f3252821a28ea6f64fdb97ce8668c4bbe4`. No production change was required.
- This issue #144 progress change is documentation-only. Executable claims are tied to the tested characterization commit above, not to the later documentation head.
- Issue #148 baseline and freshly fetched `origin/main`: `864f3d6451c240b7de9a43e84a73c673fd3c7053`.
...
That earlier beta slice established one `HtmxResponse.Trigger` family and
removed `TriggerTiming`, `TriggerAfterSwap`, and `TriggerAfterSettle`; every
convenience call merged into `HX-Trigger`. Its focused characterization covered
single and multiple events, the then-current case-insensitive duplicate
suppression, mixed details, and application `IOptions<JsonOptions>`. The fourth
#154 slice below supersedes that provisional merge and wire policy with exact,
case-sensitive names, replacement-in-place, and one safe JSON object while
retaining the timing-surface removal and htmx 4 post-swap dispatch.
...
Protected behavior for issue #135:

> When a package-consuming .NET 10 Blazor static-SSR application invokes one
> generated component action whose application-authored response contains
> ordinary main-target content followed by one sibling `<hx-partial>`, Htmxor
> preserves that response so exact application-owned htmx 4.0.0 swaps the main
> target first and the partial target afterward without changing server route,
> method, authorization, antiforgery, or fragment-selection authority.

The separate package-only application publishes in Production, starts Kestrel,
and runs exact application-owned htmx 4.0.0 in Chromium. One authorized stock
`@page` owns one generated `@onput` action. A stock full-page GET renders the
shell, one ordinary main target, one distinct partial target, and no
response-only `<hx-partial>`. Anonymous PUT receives `401`, and an authenticated
PUT with an invalid stock token receives `400`; neither changes initialization
or callback observations.

The authorized antiforgery-valid PUT initializes one request component and
invokes its sole callback once on that same instance. Its shell-free static-SSR
response contains deterministic ordinary main content followed by exactly one
raw application-authored `<hx-partial>` sibling. Response inspection proves the
sibling order and retains exact `hx-target="#issue-135-partial-target"` and
`hx-swap="innerHTML"` values. Native htmx 4 removes the delivery envelope from
ordinary main content, mutates the main target first, then mutates the partial
target. Independent `htmx:before:settle` events and body-subtree mutation
records observe the same order, and both targets finish with exact expected
content.

No Htmxor production-library change was required. The slice adds no typed
partial helper, second server fragment concept, OOB composition, renderer path,
source-generator feature, route, method, or endpoint convention. `HtmxFragment`
remains the single server-side fragment-selection concept; raw `<hx-partial>`
remains application-owned client delivery markup.

Protected behavior for issue #137:

> When a package-consuming .NET 10 Blazor static-SSR application directly
> selects one known `HtmxFragment`, Htmxor preserves one routed-page and one
> required wrapper initialization and parameter pass, renders the selected
> descendant once, and performs zero initialization, parameter, or render work
> in the excluded sibling descendant branch.

The separate package-only application publishes in Production, starts Kestrel,
and runs exact application-owned htmx 4.0.0 in Chromium. Its stock `@page` GET
retains the complete shell and renders both sibling named fragments beneath one
wrapperless application component. The routed page, wrapper, and both
descendants each initialize and receive parameters once; each descendant
records one render.

The full-page control then sends one real direct GET whose `HX-Target` identifies
the selected fragment. The response is shell-free and has exactly one
non-whitespace top-level element: the selected fragment wrapper containing the
selected descendant. It contains no control, excluded-fragment, or
excluded-descendant markup. The outerHTML swap leaves exactly one control and
one instance of each original fragment ID in the live document.

For that direct request, one trace identifier ties together exactly one routed
page initialization and parameter pass, one required wrapper initialization and
parameter pass, and one selected-descendant initialization, parameter pass, and
render observation. The excluded descendant records zero observations. This is
the slice's only skipped-work claim. It neither observes nor claims
`OnAfterRender` or `OnAfterRenderAsync`, which static SSR does not invoke.

No Htmxor production-library change was required. The existing public
`HtmxFragment` boundary already decides whether to invoke its child content.
The slice adds no renderer path, controller, Minimal API endpoint, static
component action, private reflection, copied framework code, second fragment
concept, or embedded htmx runtime.
...
- Issue #135 started from clean exact freshly fetched `origin/main` `ee52f903a3340c1acc185c2a96fb1a23b4112ca6` on branch `egil/issue-135-main-partial-order`. Live issue #135 was open and unassigned with no comments, no open pull request owned the slice, and the starting worktree was clean at the exact remote head.
- Initial green characterization at exact clean executable commit `f4b0d2abe3b51d22fb536f6058ff58953f2f8c1c`: `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Htmx4PackageBrowserTests" --blame-hang --blame-hang-timeout 5min --logger "console;verbosity=minimal"` packed unchanged Htmxor production code, restored and published the separate Production `net10.0` application, started Kestrel, and passed 1 of 1 outer tests while asserting 19 of 19 published consumer tests. The complete issue #135 scenario passed baseline Htmxor on its first execution, so no behavioral red exists and none was fabricated by omitting required application input.
- Post-Copilot focused proof at exact clean executable commit `fdf3155caef3a8d6fe9aa070030c41d29b46e124` used the same command and again passed 1 of 1 outer tests while asserting 19 of 19 published consumer tests. The case proves stock shell GET, exact application-owned htmx 4.0.0, anonymous `401`, authenticated invalid-token `400`, authorized antiforgery-valid generated PUT, exactly one initialization followed by one callback on the same request instance, exactly three top-level response elements with main content before exactly one sibling partial envelope, exact normalized response-node structure and attributes, retained envelope target and swap values, envelope extraction, exact normalized final target contents, and matching main-before-partial event and mutation order without timing sleeps. The mutation observation records transitions between recognized targets, not browser-specific consecutive `MutationRecord` cardinality. An initial sandboxed focused invocation aborted before test discovery because the test platform could not open its local socket; the unrestricted rerun above is the executable result.
- Fast-profile proof at the same exact clean post-Copilot executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile fast` passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 282 non-browser library, generator, analyzer, and runtime tests. Total: 444 discovered, 444 executed, 444 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors.
- Full-profile proof at the same exact clean post-Copilot executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile full` passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 284 library, generator, analyzer, runtime, and browser tests. Total: 447 discovered, 447 executed, 447 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors and retained fresh canonical coverage at `artifacts/results/full/htmxor/0fa30124-8129-450d-bb76-cff90d0f3443/coverage.cobertura.xml`.
- Issue #135's exact-head proof at `fdf3155caef3a8d6fe9aa070030c41d29b46e124` used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Microsoft.Playwright 1.62.0, cached Chromium revision 1234 / 151.0.7922.34, Ubuntu 26.04.1 under WSL2, exact application-owned htmx 4.0.0, a locally packed unsigned Htmxor package, a published framework-dependent `net10.0` application, Production, and Kestrel loopback HTTP. It did not exercise TLS, Windows, macOS, Firefox, WebKit, a NuGet-published or signed package, self-contained or trimmed publish, another target framework or htmx version, OOB composition, multiple or nested partials, missing or repeated targets, target fallback, another swap style, streaming, caching, history, extensions, interactive rendering, concurrency, proxies, containers, external services, or performance. Full-scope mutation was optional for this POC and was not run.
- Initial independent Standards and Spec reviews of the unpublished draft both found that selected-node assertions did not prove exact response and final-target content. Standards also rejected the draft's omitted-`<hx-partial>` scaffold as fabricated red because it did not execute the protected scenario. The live issue correction, unpublished-history rewrite around the complete passing scenario, and exact normalized assertions resolved the Standards P1/P2 and Spec P2 findings. Standards and Spec rereviews of exact head `cfdb9191cd32663dc032d66d8198a5a3ec745fbc` each passed with zero findings; Spec independently reran the focused proof with 1 of 1 outer and 19 of 19 inner tests passing, while Standards inspected the retained fast and full evidence. A wording-only correction followed at `828a24c768b9640bda213616b0e96def9cb0971d`; fresh Standards and Spec rereviews of that exact head each passed with zero findings and did not rerun executable checks. Copilot's subsequent Standards P2 found that the observer recorded one entry per `MutationRecord`, making the exact two-entry assertion depend on Chromium record cardinality. Executable commit `fdf3155caef3a8d6fe9aa070030c41d29b46e124` resolves it by de-duplicating consecutive recognized target IDs while preserving the strict main-then-partial event and mutation assertions. This pre-review progress record does not claim a Standards or Spec outcome for its own later documentation head.
- Issue #137 started from clean exact freshly fetched `origin/main` `06c2558b3bae6e19691d804382005dcd1f26738b` on branch `egil/issue-137-fragment-lifecycle`. Live issue #137 was open and unassigned with no comments.
- Green characterization at exact clean executable commit `a33d9e4635f15812329634a2f48f131e0e5ee3e4`: `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Htmx4PackageBrowserTests.Package_only_net10_production_publish_preserves_assets_and_component_actions" --blame-hang --blame-hang-timeout 10min` passed 1 of 1 outer tests while asserting 20 of 20 published consumer tests. The issue #137 case packs unchanged Htmxor production code, restores and publishes the separate `net10.0` consumer, starts Production Kestrel, and runs exact application-owned htmx 4.0.0 in Chromium. The complete lifecycle and raw-response scenario passed unchanged baseline production on its first execution, so no behavioral red exists and none was fabricated. Earlier missing-assets, sandboxed-socket, and missing consumer-namespace attempts failed before behavioral execution and are setup evidence only.
- The stock GET proof retains the HTML shell, renders both descendant outputs, and records one initialization and parameter pass for the routed page and wrapper plus one initialization, parameter pass, and render for each descendant. The direct GET carries exact partial request type and `div#issue-137-selected` target headers; its decoded response has one non-whitespace top-level selected-fragment element, no control or excluded output, and no shell. One direct-request trace identifier joins exact page, wrapper, and selected-descendant observations, while every excluded-descendant lifecycle and render count is zero. The real outerHTML swap leaves exactly one control and one selected and excluded fragment ID in the live document.
- Initial independent Standards and Spec reviews found the same P1 test-credibility gap at documentation head `f701c9842490d67f2bfc8f5773d0c6ed7e39d238`: response arrival plus text-identical pre- and post-swap DOM could let the uniqueness assertions pass before htmx completed its outerHTML swap. Commit `3b046f57dacc85e87b2a5f752eea0f6f6510b75c` renders the request trace identifier into each descendant, proves the direct response carries a different request marker, waits for that exact marker in the live DOM, and only then asserts exact final content and unique controls and fragment IDs.
- Post-review focused proof at exact clean executable commit `3b046f57dacc85e87b2a5f752eea0f6f6510b75c` used the characterization command and again passed 1 of 1 outer tests while asserting 20 of 20 published consumer tests. The exact selected response includes the new direct-request marker, and the live DOM must expose that marker before final uniqueness assertions run.
- Post-review fast-profile proof at the same exact clean executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj -- check --profile fast` passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 282 non-browser library, generator, analyzer, and runtime tests. Total: 444 discovered, 444 executed, 444 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors.
- Post-review full-profile proof at the same exact clean executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj -- check --profile full` passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 284 library, generator, analyzer, runtime, and browser tests. Total: 447 discovered, 447 executed, 447 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors and retained fresh canonical coverage at `artifacts/results/full/htmxor/45321362-d0d1-475a-a776-3fc47ec34075/coverage.cobertura.xml`.
- Issue #137's exact-head proof at `3b046f57dacc85e87b2a5f752eea0f6f6510b75c` used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Microsoft.Playwright 1.62.0, cached Chromium revision 1234 / 151.0.7922.34, Ubuntu 26.04.1 under WSL2, exact application-owned htmx 4.0.0, a locally packed unsigned Htmxor package, a published framework-dependent `net10.0` application, Production, and Kestrel loopback HTTP. It did not exercise `OnAfterRender` callbacks, TLS, Windows, macOS, Firefox, WebKit, a NuGet-published or signed package, self-contained or trimmed publish, another target framework or htmx version, nested or several selected fragments, dynamic selection, missing or repeated targets, concurrent requests, asynchronous lifecycle or quiescence, disposal, cancellation, errors, streaming, caching, forms, unsafe methods, interactive rendering, proxies, containers, external services, benchmarks, allocations, or performance. Full-scope mutation was optional for this POC and was not run.
- This issue #137 progress draft does not claim a final Standards or Spec rereview outcome. Fresh independent rereviews are required at its eventual exact clean documentation head.
- Issue #139 started from clean exact freshly fetched `origin/main` `28418a4995827a550aed1253576a1108b7c98d5a` on branch `egil/issue-139-async-fragment-quiescence`. Live issue #139 was open and unassigned with no comments.
- Green characterization at exact clean executable commit `030e3d9830dfab70022920afeac99498d9ceb85a`: `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~Htmx4PackageBrowserTests.Package_only_net10_production_publish_preserves_assets_and_component_actions --blame-hang --blame-hang-timeout 5min --logger 'trx;LogFileName=issue-139-baseline-exact-shape.trx' --results-directory artifacts/results/issue-139-baseline` passed 1 of 1 outer tests while asserting 21 of 21 published consumer tests. It packed unchanged Htmxor production code, restored and published a separate `net10.0` consumer, started Production Kestrel, and ran the complete issue #139 application-owned htmx 4.0.0 scenario in Chromium. The scenario passed unchanged baseline production, so no behavioral red exists and no production change was made. A sandboxed VSTest socket denial and an initial missing package-assets failure did not reach behavior; a later fixture-only `after:swap` target assertion failed after the completed response and DOM marker were already present and was corrected before the characterization result.
- The stock page proof retains the shell and selected and excluded placeholders while both direct-only async descendants have zero observations. During the direct GET, the selected descendant records entry into `OnInitializedAsync` and awaits a controlled gate. While blocked, the Playwright response task remains incomplete, the exact stock placeholder remains in the live target, no request-completed marker exists, no htmx `after:request` or `after:swap` event has fired, and the excluded descendant has zero lifecycle and render observations. A `finally` releases the gate. The successful shell-free decoded response then has exactly one selected fragment root with the request trace identifier as its completed marker and no stock-control, placeholder, or excluded output. Chromium must expose that exact request marker in the live DOM before final target assertions, and the ordered probe proves selected initialization completed before the browser response observation. The test intentionally makes no selected-render-count or static-SSR after-render claim.
- The initial independent Standards review at clean documentation head `bd1b55919f7f0182d0cf8b9e0a606c151202c0b8` found one P1 credibility gap: an instantaneous pending-response sample could release the gate before a prematurely emitted response won the scheduling race. The independent Spec review passed that head with zero findings. Commit `397756dd3a49b30fb56fac4c218269a26434ede8` keeps the gate closed across a bounded 500 ms scheduling opportunity before asserting the response, htmx events, DOM, and excluded work are still pending or unchanged. This interval is negative synchronization evidence, not a latency or performance claim.
- Post-review focused proof at exact clean executable commit `397756dd3a49b30fb56fac4c218269a26434ede8` used the characterization command with `issue-139-review-fix-clean.trx` under `artifacts/results/issue-139-review-fix` and passed 1 of 1 outer tests while asserting 21 of 21 published consumer tests.
- Fast-profile proof at the same exact clean post-review executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile fast` passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 282 non-browser library, generator, analyzer, and runtime tests. Total: 444 discovered, 444 executed, 444 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors.
- Full-profile proof at the same exact clean post-review executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile full` passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 284 library, generator, analyzer, runtime, and browser tests. Total: 447 discovered, 447 executed, 447 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors and retained fresh canonical coverage at `artifacts/results/full/htmxor/d002067c-3539-46f4-a696-85aa2ee131c3/coverage.cobertura.xml`.
- Issue #139's exact-head proof at `397756dd3a49b30fb56fac4c218269a26434ede8` used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Microsoft.Playwright 1.62.0, cached Chromium revision 1228 / 149.0.7827.55, Linux under WSL2, exact application-owned htmx 4.0.0, a locally packed unsigned Htmxor package, a published framework-dependent `net10.0` application, Production, and Kestrel loopback HTTP. It did not exercise TLS, Windows, macOS, Firefox, WebKit, a NuGet-published or signed package, self-contained or trimmed publish, another framework or htmx version, fresh browser provisioning, nested or several selected fragments, dynamic selection, missing or repeated targets, concurrent requests, async parameter lifecycle beyond normal initialization, disposal, cancellation, errors, streaming, caching, forms, unsafe methods, interactive rendering, proxies, containers, external services, benchmarks, allocations, throughput, or broader performance. Full-scope mutation was optional for this POC and was not run.
- This issue #139 progress draft does not claim a final Standards or Spec review outcome. Fresh independent reviews are required at the eventual exact clean documentation head.
- Issue #141 started from clean exact freshly fetched `origin/main` `a082e77c52903cbbef06d3a3e511d1a15fc41f50` on branch `egil/issue-141-remove-route-ceiling`. Live issue #141 was open and unassigned with no comments.
- Meaningful red at exact test-only commit `a3a79143ab294e4b808c3606fa54b13ca29b11bf`: `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Package_only_application_discovers_explicit_CSharp_routes_and_supported_actions" --blame-hang --blame-hang-timeout 5min --logger "console;verbosity=normal"` discovered and ran one outer test. The separately packed `net8.0` package was restored into the real external `net10.0` consumer, whose Release build failed with zero warnings and exactly three nonconfigurable `HTMXOR001` diagnostics stating that the source assembly may declare at most two `HtmxRoute` components. The diagnostics covered the new valid project-root Razor audit component plus the existing Razor-backed report and all-C# summary controls; no consumer test executed because unchanged production rejected the application at this exact arbitrary ceiling.
- The implementation removes only the analyzer and runtime-catalog count checks. The analyzer still evaluates every routed component independently, and the runtime catalog still validates the sorted manifest, project-root membership, concrete component type, exactly one route, absence of a stock route, route template and methods, generated-action consistency, authorization, and constructed metadata before mapping. No replacement count or public API was added.
- Focused analyzer, generator, and runtime proof at exact clean executable commit `a90d72e63a1a88d35da8e344584df1f0a4a112f5`: `dotnet test test/Htmxor.Tests/Htmxor.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~HtmxorRouteDeclarationAnalyzerTests|FullyQualifiedName~HtmxorRouteGeneratorTests|FullyQualifiedName~HtmxorAttributedRouteCatalogTests" --blame-hang --blame-hang-timeout 5min --logger "console;verbosity=normal"` passed 64 of 64 tests. The new controls cover eight valid analyzer declarations, four generator-manifest entries in deterministic identity order, and four runtime-catalog descriptors in deterministic identity order while the existing fail-closed cases remain green.
- Focused package proof at the same exact clean executable commit used the meaningful-red command and passed 1 of 1 outer tests while asserting 13 of 13 external consumer tests. One application-level `AddHtmxorComponentEndpoints(routes)` call maps exactly three direct-only routes in deterministic component identity order. The new Razor audit route and the existing Razor-backed report and all-C# summary routes each retain their compiled route and methods, distinct policy and host, component metadata, and shared application route-group metadata. Direct requests reach only their selected component. Normal requests, anonymous users, wrong policies, wrong hosts, and rejected integer constraints fail closed, while existing generated stock-page PUT and DELETE actions and the report PATCH action remain green.
- Fast-profile proof at the same exact clean executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile fast` passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 282 non-browser library, generator, analyzer, and runtime tests. Total: 444 discovered, 444 executed, 444 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors.
- Full-profile proof at the same exact clean executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile full` passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 284 library, generator, analyzer, runtime, and browser tests. Total: 447 discovered, 447 executed, 447 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors and retained fresh canonical coverage at `artifacts/results/full/htmxor/0b7990f0-7aee-4137-89b0-ae659f30cd67/coverage.cobertura.xml`.
- Issue #141's focused proof used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Linux under WSL2, a locally packed unsigned `net8.0` Htmxor package, an external package-only `net10.0` application, Release, and TestServer. The full profile additionally exercised the repository's existing cached Chromium browser suite, but issue #141 itself did not exercise a browser, Kestrel, Production publish, TLS, Windows, macOS, Firefox, WebKit, a NuGet-published or signed package, trimming, Native AOT, another framework, large-manifest performance, collisions, nested component directories or namespaces, multiple route attributes, or concurrent requests. Full-scope mutation was optional for this POC and was not run.
- This issue #141 progress draft does not claim a final Standards or Spec review outcome. Fresh independent reviews are required at the eventual exact clean documentation head.
- Issue #144 started from clean exact freshly fetched `origin/main` `ca5a00f3252821a28ea6f64fdb97ce8668c4bbe4` on branch `egil/issue-144-concurrent-fragment-isolation`. Live issue #144 was open and unassigned with no comments, and no other open issue owned this concurrent selected-fragment slice.
- Protected behavior: when two independent application-supplied htmx 4 partial requests select the same component-owned fragment route concurrently, Htmxor keeps each request's selected lifecycle, gate, response completion, observations, and browser swap local; releasing A completes and swaps only A while B remains pending with unchanged live DOM, releasing B then completes and swaps only B, and both excluded siblings perform zero initialization, parameter, and render work.
- Initial green characterization at exact clean executable commit `7dee48a59693c2c2f5c0e3078e4f722fc7e6e99d` passed the complete scenario against unchanged production code. No behavioral red exists and none was fabricated. An initial missing package-assets failure, a fixture compilation error, and a selected intermediate-render overassertion failed setup or test expectations rather than the protected behavior; the intermediate render occurs after asynchronous initialization yields while the response, completed marker, htmx events, and live DOM remain pending.
- Independent Standards review of `7dee48a59693c2c2f5c0e3078e4f722fc7e6e99d` found a P1 both-pending race because `Task.WhenAll` alone could hide one prematurely completed response, plus a P2 cleanup gap because dispatch and entry assertions preceded the gate-release `finally`. Independent Spec review found the same individual-response P1 and a second P1 because A was not rechecked after B completed. The amended characterization creates both gates before dispatch, encloses dispatch, entry waits, and identity assertions in the release scope, asserts each response task individually incomplete, freezes B's blocked lifecycle observations across A's release, and after B completes rechecks A's live DOM, response/swap event counts, and frozen completed lifecycle observations.
- Post-review focused proof at exact clean executable commit `36183cc64c97a55ee816e454b3d7954cfbe0c7e2`: `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Package_only_net10_production_publish_preserves_assets_and_component_actions" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue144-post-review-exact-head.trx" --results-directory artifacts/results/issue144-post-review-exact-head` passed 1 of 1 outer tests while asserting 22 of 22 published consumer tests. The harness packed unchanged Htmxor production code, restored and published a separate `net10.0` application, started Production Kestrel, and ran exact application-owned htmx 4.0.0 in real Chromium.
- Both stock full-page requests retain the HTML and application shell plus request-specific selected and excluded placeholders while recording no descendant work. Both direct requests reach the same route and selected fragment from separate Chromium pages, carry distinct application markers, request trace identifiers, selected gates, and completed markers, and enter parameter delivery plus asynchronous initialization before either release. Across a bounded negative scheduling window, both response tasks are individually incomplete, both gates remain closed, both pages retain their stock DOM, and neither page observes an htmx response or swap event.
- Releasing A completes only A's initialization and parameters, response, exact shell-free selected body, completed-marker swap, and one response plus one swap event. B's response, gate, completed phases, events, and live DOM remain pending and its lifecycle observation array remains unchanged. Releasing B then completes the corresponding B-only state. A's already-completed live DOM, absence of B identity and marker, event counts, and lifecycle observation array remain unchanged. Every observation is joined to exactly one request identity, application marker, selected gate, and selected branch; neither excluded branch records parameter, initialization, or render work.
- Fast-profile proof at the same exact clean post-review executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile fast` passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 282 non-browser library, generator, analyzer, and runtime tests. Total: 444 discovered, 444 executed, 444 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors.
- Full-profile proof at the same exact clean post-review executable commit: `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj --configuration Release --no-restore -- check --profile full` passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 284 library, generator, analyzer, runtime, and browser tests. Total: 447 discovered, 447 executed, 447 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The authoritative Release build produced 0 warnings and 0 errors and retained fresh canonical coverage at `artifacts/results/full/htmxor/98242d20-e6c9-4f43-8618-68dde058fc1b/coverage.cobertura.xml`.
- Issue #144's exact-head proof used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Microsoft.Playwright 1.62.0, cached Chromium revision 1234 / 151.0.7922.34, Ubuntu 26.04.1 under WSL2, exact application-owned htmx 4.0.0, a locally packed unsigned `net8.0` Htmxor package, a published framework-dependent `net10.0` application, Production, and Kestrel loopback HTTP. It did not exercise TLS, Windows, macOS, Firefox, WebKit, fresh browser provisioning, a NuGet-published or signed package, self-contained or trimmed publish, another framework or htmx version, repeated requests in one page, nested or multiple selection within one response, cancellation, disposal, errors, forms, unsafe methods, streaming, caching, interactive rendering, proxies, containers, external services, benchmarks, allocations, throughput, or broader performance. Full-scope mutation was optional for this POC and was not run.
- Final independent Standards and Spec rereviews of exact clean executable head `36183cc64c97a55ee816e454b3d7954cfbe0c7e2` each passed with 0 findings and worst priority none. Both inspected the full five-file `+404/-1` test-only diff, `git diff --check`, and the supplied exact-head focused, fast, full, and coverage evidence; neither independently reran tests. This later progress change is documentation-only and requires final review at its own exact clean head.

## Remaining limits

- Issues #95, #97, #100, #103, #106, #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, #139, #141, and #144 prove one locally packed package with the current SDK and dependency set. Issues #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, #139, and #144 additionally prove a framework-dependent application publish, but none proves a NuGet-published or signed package, a release candidate, package compatibility across SDK or compiler versions, or a broader target-framework matrix.
...
- The authorization proof uses one deterministic scheme and one claim policy. It covers the earlier GET path, issue #120, #133, and #135 PUT paths, and issue #131's DELETE path, but not scheme selection, custom challenge or forbid handlers, identity-provider integration, or other HTTP methods.
...
- Issues #91, #93, #95, #97, #100, and #103 ran their hosted contract only on Windows TestServer. Issue #141 ran its hosted contract on Linux under WSL2 TestServer. These slices did not exercise Kestrel, TLS, a browser, or an application-selected HTMX runtime. Issue #108 adds one package-only Kestrel and Chromium GET path on Windows and Linux, but it does not prove those earlier package-only routes and actions in a browser.
- Beyond issue #108's GET, issue #56's unsafe-action matrix, issue #111's QUERY slice, issue #50's server OutputCache slice, issue #122's pure multi-target fragment response, issue #64's full-page redirect navigation, issue #129's application-owned error response, issue #131's native DELETE placement, issue #133's single mixed main/OOB response, issue #135's single mixed main/partial response, issue #137's synchronous selected-fragment lifecycle case, issue #139's asynchronous selected-fragment quiescence case, and issue #144's concurrent selected-request isolation case, the htmx 4 browser tests do not exercise layouts, browser caching, enhanced navigation, interactive render modes, broader fragment shapes, other OOB or partial shapes, history, extensions, or performance.
...
- Issues #108, #56, #111, #50, #116, #118, #120, #64, #129, #131, #133, #135, and #137 cover application-owned htmx
  ...
  target remains unchanged. Issue #135 proves one mixed ordinary main response
  followed by one raw partial envelope using an explicit target and `innerHTML`.
  Target fallback from an envelope `id`, OOB plus partial composition, missing
  targets, repeated targets, and other mixed-response swap styles remain unproved.
...
- Issue #135 proves one generated stock-page PUT action returning one ordinary
  main result followed by one sibling raw `<hx-partial>` with explicit
  `hx-target` and `hx-swap="innerHTML"` for an existing distinct target. It
  proves exact response order and attributes, final content, envelope extraction,
  and main-before-partial event and mutation order. It does not prove OOB plus
  partial composition, multiple or nested partials, missing or repeated targets,
  target fallback, another swap style, method, framework, or htmx version,
  streaming, caching, history, extensions, interactive rendering, concurrency,
  or performance.
- Issue #137 proves one stock-page direct GET selecting one of two sibling named
  `HtmxFragment` boundaries beneath one wrapperless application component. It
  proves exact synchronous initialization and parameter counts for the routed
  page and wrapper, exact synchronous initialization, parameter, and render
  counts for both descendants on the stock GET and only the selected descendant
  on the direct GET, and zero work in the excluded descendant. It does not prove
  fragment-boundary lifecycle avoidance, nested boundaries, several selections,
  dynamic selection, missing or repeated targets, concurrent requests, disposal,
  cancellation, errors, streaming, caching, forms, unsafe methods, interactive rendering, another
  framework, htmx version, browser, operating system, or performance. Static SSR
  after-render callbacks were not observed and are not an optimization claim.
- Issue #139 proves one stock-page direct GET selecting a direct-only fragment
  whose descendant blocks in `OnInitializedAsync`. It proves that the response
  task and htmx response/swap events remain incomplete while the gate is closed,
  the stock placeholder remains, the excluded direct-only sibling records zero
  lifecycle and render work, selected initialization completes before browser
  response observation, and the request-unique completed marker reaches the live
  DOM. It does not prove a fixed selected-render count, `OnAfterRender`, async
  parameter work beyond normal initialization, nested or multiple selection,
  dynamic or repeated targets, concurrent requests, disposal, cancellation,
  errors, streaming, caching, forms, unsafe methods, interactive rendering,
  another framework, htmx version, browser, operating system, or performance.
- Issue #144 proves exactly two concurrent direct GETs from separate Chromium
  pages selecting the same direct-only fragment beneath one stock `@page`.
  It proves distinct request identities, application markers, selected gates,
  lifecycle records, completed markers, response completion, response/swap
  events, and live DOM; staged releases cannot complete, mutate, or add
  observations to the other request, and each excluded sibling records zero
  initialization, parameter, and render work. It does not prove more than two
  concurrent requests, repeated requests in one page, fixed selected-render
  counts, nested or multiple selection within one response, dynamic or repeated
  targets, cancellation, disposal, errors, streaming, caching, forms, unsafe
  methods, interactive rendering, another framework or htmx version, another
  browser or operating system, proxies, containers, benchmarks, allocations,
  throughput, or broader performance.
...
- The issue #97 follow-up removes Razor-text interpretation. Its path-only
  ...
  future SDK or analyzer-pipeline changes, multiple routes on one component,
  collision policy, normal-only or dual
...
- The analyzer activates for every application that restores the package.
  Issue #141 removes the arbitrary source-assembly count rejection while every
  declaration outside the proved project-root route and authorization contract
  still receives a build error. The runtime catalog also scans the application
  assembly and fails closed if compiled declarations do not match the generated
  manifest. Three project-root package-consumer components are proved, with
  eight analyzer declarations and four generated-manifest and runtime-catalog
  entries as above-two controls. Very large manifests and their startup cost,
  multiple route attributes on one component, collision policy, nested
  namespaces, broader route filters, broader unsafe action discovery, IDE
  live-analysis parity, trimming, and Native AOT remain unproved. This fail-fast
  behavior is accepted for the locally packed beta spike: unrelated unrouted
...
> When a developer installs Htmxor v1, the package, generator, samples, tests,
> CI/tooling, README, and v1 contract consistently target .NET 10 and use
> supported ASP.NET Core 10 APIs; nothing advertises or retains .NET 8
> compatibility. .NET 11 remains unclaimed until separately executed.

The migration is behavior-preserving at the package-consumer boundary, so its
alternate baseline is the green package-only test at exact `origin/main`
`864f3d6451c240b7de9a43e84a73c673fd3c7053`: one outer test passed while
asserting 13 of 13 separately restored `.NET 10` consumer tests against the
then-`net8.0` package. Compilation failures caused solely by changing a target
framework were not treated as meaningful red.

The first complete fast profile at exact clean commit
`24119381f3d1c88aeeded651645e2712bba51bb8` recorded meaningful .NET 10 runtime
red. The quality and ASP.NET Core 10 projects passed 117 of 117 and 45 of 45
...
The first bounded slice of issue #151 establishes the current registration
names:

> When a .NET 10 Blazor static-SSR application calls `AddHtmxor()` and maps Razor
> components at the root or through one standard ASP.NET Core route group with
> `AddHtmxorEndpoints()`, Htmxor enables its generated routes while preserving
> the same prefix, conventions, metadata, method and security constraints, and
> stock-page behavior, with no unprefixed duplicate.
...
At issue #145's exact clean executable commit
...
The naming slice does not expand issue #145's mapping coverage. Nested route
groups, multiple separately mapped Razor component applications, group endpoint
filters or rate limiting, interactive render modes, another ASP.NET Core
...
### Conservative htmx request marker classification
...
At exact clean test-only commit
`8ad87a972520d5d407b9fccd372532cb0412ec41`, the focused packed-package command
passed 22 and failed 12 of 34 separately compiled and executed external-consumer
tests. Invalid markers reached the generated HTMX-only route, selected direct
output for the dual `@page` route, or allowed guarded response status, header,
and body-state changes. Missing and blank markers retained stock routing but
exposed the missing raw `Reswap(string)` guard. These were behavioral failures
...
- focused `HtmxRequestTests` and `HtmxResponseTests`: 41 discovered, executed,
  and passed;
- separately packed `net10.0` consumer: 1 outer test passed while asserting 34
  of 34 external consumer tests, including invalid-marker HTMX-only and dual
  route behavior plus all covered response operations;
- relevant ASP.NET Core 10 stock-page and HTMX-only routing tests: 27 of 27
  passed;
- fast profile: 431 of 431 passed (117 quality, 45 ASP.NET Core 10, and 269
  core tests), with analyzer and style error gates and the Release build green;
  and
...
This proof used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Linux under WSL2, a
locally packed unsigned `net10.0` package, and TestServer. The full profile's
existing cached-Chromium suite is a valid-marker regression check; it does not
prove malformed-marker browser behavior. This slice did not exercise TLS,
Windows, macOS, Firefox, WebKit, fresh browser or SDK provisioning, a published
or signed package, .NET 11, another framework or htmx version, proxies,
containers, external services, or performance. Full-scope mutation was not run
and is not required for this bounded slice.
...
### Deterministic trigger response contract

The fourth bounded slice of issue #154 protects this behavior:

> When a valid htmx 4 component response triggers events across one or more
> fluent calls, Htmxor emits one deterministic `HX-Trigger` JSON object whose
> exact, case-sensitive event entries retain first wire position while later
> duplicates replace their details, without changing status or body; invalid,
> unserializable, or invalid-marker calls leave the response untouched.

The exact freshly fetched base and merge base were
`5ece9bb1d749cbd5d00ac20ddc1e869c49dc605a`. Live issue #154 was open and
assigned to `egil`, and no open pull request overlapped this bounded trigger
slice.

Test-only commit `a4fcb310555bf67e12e00315c32019c94a90eb6b`
preserves meaningful behavioral red. The focused Trigger selection compiled,
discovered, and executed 18 tests: 6 passed and 12 failed on the old comma/null
wire forms, unsafe property-name interpolation, case-insensitive duplicate
suppression, first-call-wins details, marker-before-serialization ordering, and
manual-header ownership. The separately packed `net10.0` application compiled,
started TestServer, and executed all 62 inner tests: 58 passed and 4 failed on
the corresponding package API, validation-order, status/body, and component
response behavior. Its outer test failed only because those four inner
behavioral assertions failed.

The first focused red attempt was a setup failure: the restricted environment
denied MSBuild's named-pipe socket, so it is not red evidence. Repeating the
same command with the required process permissions produced the 18-test result
above. The exact red commands were:

```text
dotnet test test/Htmxor.Tests/Htmxor.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Trigger" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-trigger-red-focused.trx" --results-directory artifacts/results/issue-154-trigger-red-focused --verbosity minimal
dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~PackedPackageConsumerTests.Package_only_application_discovers_explicit_CSharp_routes_and_supported_actions --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-trigger-red-packed.trx" --results-directory artifacts/results/issue-154-trigger-red-packed --verbosity minimal
```

Implementation and public-documentation commit
`f0426fca0e0abc8a5ad2ea98e9ae5043fa5a0704` validates event names before the
...
Success returns the same `HtmxResponse` and preserves status, declared body
state, explicit `EmptyBody()`, and navigation suppression. The htmx 2 timing
type, overload, and timed header constants remain absent. The packed routes are
still component-owned Razor routes and callbacks; this slice adds no controller,
Minimal API endpoint, static handler, renderer copy, private reflection, or
embedded htmx runtime.

The [live issue clarification](https://github.com/egil/Htmxor/issues/154#issuecomment-5493907919)
requested JSON null for an event without detail, but exact htmx 4.0.0 does not
dispatch that representation. Its
[header handler](https://github.com/bigskysoftware/htmx/blob/v4.0.0/src/htmx.js#L859-L873)
passes parsed null as an object detail, and the
[event path](https://github.com/bigskysoftware/htmx/blob/v4.0.0/src/htmx.js#L1565-L1575)
dereferences `detail.cancelled` before dispatch. This slice therefore emits
`{}` for no detail, matching the empty detail shape of htmx's comma form and
preserving the observable dispatch requirement. JSON objects remain
semantically unordered, and htmx's JavaScript enumeration can reorder
...
- the actionless separately packed command passed 1 of 1 outer tests while
  asserting 65 of 65 inner TestServer tests;
- the package-only Production Kestrel/Chromium command passed 1 of 1 outer
  tests while asserting 34 of 34 inner tests. The Trigger interaction retained
  `202 Accepted`, replaced a manually seeded header with one exact compact
  object, safely encoded a quote/backslash event name, and dispatched four
  non-index events after the visible swap. The duplicate fired once with its
  last detail at its first position; no timed header, manual event, page error,
  or console error appeared;
- the fast profile passed 472 of 472 tests: 117 quality, 45 ASP.NET Core 10,
  and 310 core tests; and
- the full profile passed 475 of 475 tests: 118 quality, 45 ASP.NET Core 10,
  and 312 core/browser tests. Analyzer and style error gates passed, the Release
...
dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~PackedPackageConsumerTests.Package_only_application_discovers_explicit_CSharp_routes_and_supported_actions --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-trigger-final-packed.trx" --results-directory artifacts/results/issue-154-trigger-final-packed --verbosity minimal
dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~PackedPackageConsumerTests.Package_only_application_preserves_actionless_unsafe_route_antiforgery --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-trigger-final-packed-actionless.trx" --results-directory artifacts/results/issue-154-trigger-final-packed-actionless --verbosity minimal
dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~Htmx4PackageBrowserTests.Package_only_net10_production_publish_preserves_assets_and_component_actions --blame-hang --blame-hang-timeout 10min --logger "trx;LogFileName=issue-154-trigger-final-htmx4.trx" --results-directory artifacts/results/issue-154-trigger-final-htmx4 --verbosity minimal
dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj -- check --profile fast
dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj -- check --profile full
```

One mistyped exploratory actionless filter discovered zero tests and is not
evidence; the correctly named command above discovered and passed its outer
test and asserted all 65 inner tests. The fast and full restores and Release
builds reported four `NU1900` warnings because the environment could not reach
NuGet's vulnerability feed. The quality profiles still completed, but package
vulnerability audit data is unavailable rather than clean or zero.

This proof used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Ubuntu 26.04.1 LTS
under WSL2, locally packed unsigned `net10.0` packages, TestServer,
framework-dependent Production publish, loopback HTTP with real Kestrel,
Microsoft Playwright 1.62.0, cached Chromium revision 1234 / Chrome for Testing
151.0.7922.34, and the exact application-owned htmx 4.0.0 asset with SHA-256
`E484D9171A9DB30A39C8F16E3D709D4137F3211C659F8E6125816635033D593F`.

It does not exercise fresh browser, SDK, or NuGet-audit provisioning, TLS,
Windows, macOS, Firefox, WebKit, a NuGet-published or signed package,
self-contained or trimmed publish, .NET 11, another framework or htmx version,
integer-like event dispatch order, the protocol-significant detail `target`,
concurrent Trigger calls, response mutation after start, 3xx header processing,
...
`a60a46f90f7ce59238caebe93ec8ea45b6bc1438` (`fix(protocol): preserve unicode
trigger event names`). The implementation change is deliberately narrow:
accepted navigation and swap/selection response values must be ASCII
HTTP-header-safe text, trigger event names retain well-formed Unicode JSON
property-name support, and the existing validation-before-marker ordering and
...
- `HtmxResponseHeaderNames` exposes exactly the nine core response names:
  ...
  only themselves; trigger calls own one merged compact JSON object and preserve
  exact well-formed Unicode event names; guarded
...
- The packed package consumer sends the seven core fields over `HttpClient` to
  ...
  values and the stock/direct representation.
...
public 7+9 reference table, rejects non-ASCII navigation and swap/selection
values before they can reach response-header mutation or Kestrel serialization,
and preserves Unicode trigger event names for JSON encoding.
...
- The review-exposed trigger regression was reproduced by temporarily restoring
  the pre-fix shared open-value validation on the otherwise clean `a60a46f`
  checkout while adding only the new regression test. The focused command failed
  1 of 1 because the Unicode event name was rejected as a direct response-header
  value instead of being JSON encoded. The retained red TRX is
  `artifacts/results/issue-154-unicode-trigger-red/issue-154-unicode-trigger-red.trx`.

  ```text
  dotnet test test/Htmxor.Tests/Htmxor.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~HtmxResponseTests.Trigger_preserves_unicode_event_name_as_a_json_property" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-unicode-trigger-red.trx" --results-directory artifacts/results/issue-154-unicode-trigger-red --verbosity minimal -m:1
  ```

Evidence at the exact clean corrected executable head
`a60a46f90f7ce59238caebe93ec8ea45b6bc1438`:

- Focused HTTP/route characterization:
  ```text
  dotnet test test/Htmxor.Tests/Htmxor.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~HtmxRequestTests|FullyQualifiedName~HtmxResponseTests|FullyQualifiedName~HtmxorComponentEndpointMatcherPolicyTest|FullyQualifiedName~HtmxExtensionHeaderTests|FullyQualifiedName~HtmxRouteAttributeTests" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-review-remediation-focused.trx" --results-directory artifacts/results/issue-154-review-remediation-focused --verbosity minimal -m:1
  ```
  passed 151/151; its retained TRX is
  `artifacts/results/issue-154-review-remediation-focused/issue-154-review-remediation-focused.trx`.
  The trigger-specific green replay also passed 1/1; its retained TRX is
  `artifacts/results/issue-154-unicode-trigger-green/issue-154-unicode-trigger-green.trx`.
- Separately packed package consumers passed one outer test each. The exact
  standard-consumer command was:

  ```text
  dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~PackedPackageConsumerTests.Package_only_application_discovers_explicit_CSharp_routes_and_supported_actions" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-review-remediation-packed-green.trx" --results-directory artifacts/results/issue-154-review-remediation-packed-green --verbosity minimal -m:1
  ```
  ...
  ```text
  dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~PackedPackageConsumerTests.Package_only_application_preserves_actionless_unsafe_route_antiforgery" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-review-remediation-packed-actionless-green.trx" --results-directory artifacts/results/issue-154-review-remediation-packed-actionless-green --verbosity minimal -m:1
  ```

  It passed its outer test and parsed 96/96 inner cases. Their retained TRX
  files are `artifacts/results/issue-154-review-remediation-packed-green/
  issue-154-review-remediation-packed-green.trx` and
  `artifacts/results/issue-154-review-remediation-packed-actionless-green/
  issue-154-review-remediation-packed-actionless-green.trx`.
- The repository-owned public-surface verifier passed 1/1 outer test. Its exact
  command was:

  ```text
  dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~PackedPackageConsumerTests.Package_only_public_surface_matches_retained_allow_list" --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue-154-review-remediation-public-surface.trx" --results-directory artifacts/results/issue-154-review-remediation-public-surface --verbosity minimal -m:1
  ```
  ...
  `artifacts/results/issue-154-review-remediation-public-surface/issue-154-review-remediation-public-surface.trx`.
- Separately packed `net10.0` Production/Kestrel/Chromium proof passed one outer
  test; the exact browser command was:

  ```text
  dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter "FullyQualifiedName~Htmx4PackageBrowserTests.Package_only_net10_production_publish_preserves_assets_and_component_actions" --blame-hang --blame-hang-timeout 10min --logger "trx;LogFileName=issue-154-review-remediation-browser.trx" --results-directory artifacts/results/issue-154-review-remediation-browser --verbosity minimal
  ```
  ...
  `artifacts/results/issue-154-review-remediation-browser/issue-154-review-remediation-browser.trx`.
- The repository-owned command
  `dotnet run --project eng/Htmxor.Quality/Htmxor.Quality.csproj -- check --profile fast`
  passed 118 quality, 45 ASP.NET Core 10 hosted, and 362 non-E2E Htmxor tests:
  525/525 total, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The same
  command with `--profile full` passed 119 quality, 45 hosted, and 364 complete
  Htmxor tests: 528/528 total, with the same zero failure/skip/error/timeout
...
- Full coverage retained two fresh nonempty byte-identical Cobertura copies with
  SHA-256
  `65285c87f227650e14957c2f33bb64096141297e544c167ab10199faf06d2f57` at
  `artifacts/results/full/htmxor/cfbcd1e8-5c66-4221-81e0-5f47ae5235d4/
  coverage.cobertura.xml` and
  `artifacts/results/full/htmxor/_eagle1-wsl_2026-09-02_21_20_47/In/eagle1-wsl/
  coverage.cobertura.xml`. Each file is 906083 bytes; this is coverage
...
- The packed audit command was:
  ...
  `2043f1f595a7ed9eaa5d3b710f01e244627d342ef13b185bdb834420e8c5275f`; it
