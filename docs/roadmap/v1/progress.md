...
- Current implementation slice: issue #139, one selected asynchronous fragment lifecycle-quiescence characterization through the locally packed Production Kestrel/Chromium boundary.
...
- Issues #95, #97, #100, #103, #106, #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, and #139 prove one locally packed package with the current SDK and dependency set. Issues #72, #75, #116, #118, #120, #122, #64, #127, #129, #131, #133, #135, #137, and #139 additionally prove a framework-dependent application publish, but none proves a NuGet-published or signed package, a release candidate, package compatibility across SDK or compiler versions, or a broader target-framework matrix.
...
Issue #139 establishes the current selected-fragment asynchronous lifecycle
contract:

> When a package-consuming .NET 10 Blazor static-SSR application directly
> selects one known `HtmxFragment` whose descendant performs asynchronous
> initialization, Htmxor waits for that selected lifecycle work before the
> response reaches htmx, while the excluded sibling descendant never starts its
> lifecycle or render work.

The published Production package consumer proves a stock shell page whose
direct-only fragment descendants do not start, one real htmx 4 GET targeting the
selected boundary, the still-pending response and unchanged placeholder while
selected initialization is gated, zero excluded work, a finally-safe gate
release, an exact shell-free selected-only response with a request-unique
completed marker, selected completion before browser response observation, and
that exact marker in the final live DOM through Kestrel and Chromium. The
existing public fragment boundary already satisfies this contract; no Htmxor
production or public API change was required.

The recommended next slice is a separately scoped concurrent-request isolation
proof at the same package/Kestrel/Chromium boundary. It should use two
