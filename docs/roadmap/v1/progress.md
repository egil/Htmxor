...
- This issue #122 progress change is documentation-only. Executable claims are tied to the tested proof head above, not to this later documentation head.
- Framework boundary under test: ASP.NET Core 10.0.11 and Blazor static SSR. Issues #95, #97, #100, #103, and #106 use a separate external .NET 10 Razor consumer on TestServer that restores a locally packed `net8.0` Htmxor package instead of referencing an Htmxor project. Issues #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, and #122 use a separate package-only .NET 10 application on real Kestrel; its browser cases use Chromium, while issues #50 and #18 use `HttpClient` for their server-response assertions. Issues #72, #75, #116, #118, #120, and #122 publish that external application before running its tests from the publish output in Production.
...
- Current implementation slice: issue #122, `HtmxFragment` composition with htmx 4 `<hx-partial>` delivery.
...
- Fast-profile proof at the same clean executable head passed 117 quality tests, 45 ASP.NET Core 10 hosted tests, and 268 non-browser library, generator, analyzer, and runtime tests. Total: 430 discovered, 430 executed, 430 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The Release build produced 0 warnings and 0 errors.
- Full-profile proof at the same clean executable head passed 118 quality tests, 45 ASP.NET Core 10 hosted tests, and all 270 library, generator, analyzer, runtime, and legacy-browser tests. Total: 433 discovered, 433 executed, 433 passed, 0 failed, 0 skipped, 0 errors, and 0 timeouts. The Release build produced 0 warnings and 0 errors and retained the fresh canonical coverage report at `artifacts/results/full/htmxor/d691c4e8-6b60-43a1-84b3-78d33f6b0a55/coverage.cobertura.xml`.
- Issue #122's pre-review exact-head proof used .NET SDK 10.0.400, ASP.NET Core 10.0.11, Microsoft.Playwright 1.62.0, cached Chromium revision 1234 / 151.0.7922.34 on Ubuntu 26.04 under WSL2, exact application-owned htmx 4.0.0, a locally packed unsigned Htmxor package, a published framework-dependent `net10.0` application, Production, and Kestrel loopback HTTP. It did not exercise TLS, Windows, macOS, Firefox, WebKit, a NuGet-published or signed package, self-contained or trimmed publish, another target framework or htmx version, fresh browser provisioning, reverse proxies, containers, external services, redirects, unsafe methods, error swapping, caching interaction, streaming, lifecycle or excluded-work performance, exhaustive ordering or out-of-band behavior, or full-scope mutation. Mutation was optional for this POC.

## Remaining limits

- Issues #95, #97, #100, #103, #106, #108, #56, #111, #50, #18, #72, #75, #116, #118, #120, and #122 prove one locally packed package with the current SDK and dependency set. Issues #72, #75, #116, #118, #120, and #122 additionally prove a framework-dependent application publish, but none proves a NuGet-published or signed package, a release candidate, package compatibility across SDK or compiler versions, or a broader target-framework matrix.
...
- Beyond issue #108's GET, issue #56's unsafe-action matrix, issue #111's QUERY slice, issue #50's server OutputCache slice, and issue #122's pure multi-target fragment response, the htmx 4 browser tests do not exercise layouts, browser caching, concurrency, enhanced navigation, interactive render modes, broader fragment shapes, out-of-band content, history, extensions, or performance.
...
Issue #122 establishes the current fragment-delivery contract:

> When a package-consuming .NET 10 static-SSR component selects multiple
> server-owned `HtmxFragment` instances, exact application-owned htmx 4.0.0
> receives only `<hx-partial>` delivery envelopes, updates both declared browser
> targets, and leaves the ordinary main target unchanged while a stock full-page
> GET remains ordinary markup.

`HtmxFragment` is the only server-side selection boundary. It is wrapperless by
default and can emit a supplied element, identifier, and unmatched HTML
attributes. `<hx-partial>` remains delivery-only, whether authored through
`Element="hx-partial"` or as raw application markup inside a fragment. The real
package, published Production application, Kestrel, exact htmx 4.0.0, response
assertions, and Chromium prove the narrow pure multi-target composition.

The recommended next slice is redirect and navigation behavior in issue #64.
Its protected behavior should distinguish normal navigation from direct htmx
requests and prove the chosen HTTP response plus browser URL/DOM outcome through
the locally packed package, published Production application, real Kestrel,
exact application-owned htmx 4.0.0, and Chromium. Error-response swapping,
caching expansion, mixed main/OOB/partial ordering, streaming, lifecycle and
excluded-work performance, attribute-order-independent action discovery, typed
HTMX-only route conversion, and positive omitted-`Methods` inference remain
separate slices.
