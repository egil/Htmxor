...
- Verified executable characterization for issue #120: `7b53152b4a8c5c16ccaed8eda363b2a9aebd1327`, based on exact fetched `origin/main` commit `2ec870b629e6aeb8b2f56f62397d12c343ba2e45`. No production change was required.
...
- The final supported-boundary characterization at clean exact executable head `7b53152b4a8c5c16ccaed8eda363b2a9aebd1327` required no production change. `dotnet test test/Htmxor.Quality.Tests/Htmxor.Quality.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~Htmx4PackageBrowserTests.Package_only_net10_production_publish_preserves_assets_and_component_actions --blame-hang --blame-hang-timeout 5min --logger "trx;LogFileName=issue120-characterization-green.trx" --results-directory artifacts/results/issue120-characterization-green` discovered, executed, and passed 1 of 1 outer tests; its asserted external-consumer TRX discovered, executed, and passed 11 of 11 published Production tests.
- JavaScript-disabled Chromium submitted native POST `/issue-120/native/7`, invoked the stock page's `@onpost` callback once, retained antiforgery, and received stock full-page output. Exact application-owned htmx 4.0.0 submitted PUT `/issue-120/enhanced/7` with `HX-Request-Type: partial`, invoked the separate authorized HTMX-only component's omitted-`Methods` `@onput` callback once, retained the adapter-supplied antiforgery header, swapped shell-free component output into the intended target, and left the browser URL on the native page. The two callback observations had distinct request identities and exact owner, method, and path values.
...
- Issues #108, #56, #111, #50, #116, #118, and #120 cover application-owned htmx
  ...
  using htmx 4 defaults. Other response headers, explicit inheritance, error-response swapping,
...
Issue #120 establishes the current htmx 4 progressive-form contract:

> When a package-consuming .NET 10 static-SSR page renders distinct native POST
> and htmx PUT form destinations, Htmxor preserves the stock full-page fallback
> and the enhanced partial swap while component bindings remain the only server
> route and method intent.

The supported v1 developer model keeps route ownership separate: the stock
`@page` component owns the native POST and `@onpost`, while a component without
`@page` owns the omitted-`Methods` `HtmxRoute` and `@onput`. The form's raw
`action`/`method` and `hx-action`/`hx-method` attributes choose between those
already-declared paths; they create no route, method, action, authorization, or
antiforgery grant. The real package, published Production application, Kestrel,
exact htmx 4.0.0, and Chromium proof covers both representations and fail-closed
controls. Existing production behavior already satisfied this composition.

The recommended next slice is `HtmxFragment` composition with htmx 4
`<hx-partial>` browser behavior and ordinary Razor DX evaluation. Its protected
behavior should be: when a component selects one or several server-owned
fragments, exact application-owned htmx 4 receives and swaps pure or
multi-target partial envelopes without Htmxor bundling the runtime or creating
a second server-side selection concept. The slice should evaluate whether a
beta-breaking `HtmxPartial` markup adapter replacing `HtmxFragmentElement`
materially improves ordinary Razor call sites while `HtmxFragment` remains the
single selection boundary. Redirect and navigation behavior in #64 follows
that slice. Attribute-order-independent action discovery and typed HTMX-only
route conversion remain explicit later focused candidates; error-response
swapping, other response headers, caching expansion, and positive
omitted-`Methods` inference remain separate.
