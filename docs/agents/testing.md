...
The target-aware upstream-monitor fixtures in `fast` and `full` require both the repository-selected .NET 10 SDK and .NET 11 RC1 SDK `11.0.100-rc.1.26425.128`, including its ASP.NET Core reference pack. CI provisions both; local contributors must install that exact RC SDK before running either profile.
