...
internal static class HtmxorComponentResultExecutor
{
	...
	{
		var endpointHtmlRenderer = httpContext.RequestServices.GetRequiredService<HtmxorRenderer>();
		return endpointHtmlRenderer.Dispatcher.InvokeAsync(async () =>
		{
			...
			var htmlContent = await endpointHtmlRenderer.RenderEndpointComponent(httpContext, typeof(HtmxorComponentEndpointHost), hostParameters);
