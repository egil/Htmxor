...
[assembly: MetadataUpdateHandler(typeof(HtmxorComponentState))]

namespace Htmxor.Rendering;

internal sealed class HtmxorComponentState : ComponentState
{
    private static readonly ConcurrentDictionary<Type, StreamRenderingAttribute?> _streamRenderingAttributeByComponentType = new();

    public HtmxorComponentState(Renderer renderer, int componentId, IComponent component, ComponentState? parentComponentState)
        : base(renderer, componentId, component, parentComponentState)
    {
        ...
        {
            var parentEndpointComponentState = (HtmxorComponentState?)LogicalParentComponentState;
