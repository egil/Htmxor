...
using Htmxor.FormMapping.Converters.DictionaryAdapters;
        ...
        foreach (var key in keys)
        {
            ...
            if (!TKey.TryParse(key[1..^1].Span, CultureInfo.InvariantCulture, out var keyValue))
            {
                ...
                context.AddMappingError(
                    FormattableStringFactory.Create("The value '{0}' is not a valid key for '{1}'.", key[1..^1], currentPrefix),
                    null);
            ...
            {
                context.AddMappingError(
                     FormattableStringFactory.Create("The number of elements in the {0} exceeded the maximum number of '{1}' elements allowed.", "dictionary", maxCollectionSize),
                     null);
                succeded = false;
                break;
            }
